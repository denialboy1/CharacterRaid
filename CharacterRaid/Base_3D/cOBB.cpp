...
void cOBB::Setup(SkinnedMesh* pSkinnedMesh)
{
...
void cOBB::Setup(D3DXVECTOR3 _min, D3DXVECTOR3 _max){
...
{
	GameManager::Get().GetDevice()->SetTransform(D3DTS_WORLD, &m_matWorldTM);
	GameManager::Get().GetDevice()->SetRenderState(D3DRS_LIGHTING, false);
	...
	GameManager::Get().GetDevice()->SetRenderState(D3DRS_LIGHTING, false);
	GameManager::Get().GetDevice()->SetTransform(D3DTS_WORLD, &m_matWorldTM);
	GameManager::Get().GetDevice()->SetFVF(ST_PC_VERTEX::FVF);
	GameManager::Get().GetDevice()->DrawIndexedPrimitiveUP(D3DPT_LINELIST,
		0,
		8,
		12,
		&vecIndex[0],
		...
		&vecVertex[0],
		sizeof(ST_PC_VERTEX));
	GameManager::Get().GetDevice()->SetRenderState(D3DRS_LIGHTING, true);
}
