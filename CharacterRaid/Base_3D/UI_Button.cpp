...
void UI_Button::SetTexture(char* normalName, char* overName, char* pressName)
{
	...
	textures[ButtonState::STATE_NORMAL] = TextureManager::GetTexture(normalName, &imageInfo);
	size.x = (float)imageInfo.Width;
	size.y = (float)imageInfo.Height;
	textures[ButtonState::STATE_OVER] = TextureManager::GetTexture(overName);
	textures[ButtonState::STATE_PRESS] = TextureManager::GetTexture(pressName);
}
...
LRESULT UI_Button::InputProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	if ( result == 0 )
	{
		...
		switch ( message )
		{
			...
			case WM_LBUTTONUP:
				{
					if ( state == ButtonState::STATE_PRESS )
					{
						POINT mousePoint;
						mousePoint.x = GET_X_LPARAM(lParam);
						mousePoint.y = GET_Y_LPARAM(lParam);
			...
			case WM_MOUSEMOVE:
				{
						...
						&& mousePoint.y > buttonRect.top && mousePoint.y < buttonRect.bottom )
					{
						...
						{
							state = ButtonState::STATE_OVER;	//밖에서 처음으로 안으로 들어온 상황
						}
					}
					else
					{
						...
						{
							state = ButtonState::STATE_NORMAL;	//안에서 처음으로 밖으로 들어온 상황
						}
					}
					//result = WM_MOUSEMOVE;
				}
